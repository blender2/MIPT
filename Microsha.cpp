...
void init_redirections(
    ...
    if (conveer.size() == 1) {
        ...
        if (redirs.size() == 2) {
            if (redirs[0].type == 1) swap(redirs[0], redirs[1]); //[0] -- stdin, [1] -- stdout
        }
        return;
    }

    //может ли быть что то типа: cat < a > b | echo 123 | cat < a > b ?
    //сейчас микроша обработает только первые перенаправления. Пример обработки верхней строки: cat < a | echo 123 | cat < a
...
{   
	...
	for (int i = 0; i < args_ch.size(); i++) {
        ...
        if (pid == 0) {
            ...
            if (contains_redir_2(i, redirs)) {
                if (n == 1) {
                    if (redirs.size() == 2) {
                        if (redirs[0].type == redirs[1].type) {
                            fprintf(stderr, "bad redirection\n");
                            exit(0);
                        }
                        ...
                        if (redirs[0].type == 0)
                        {
                            ...
                            fd2 = open(redirs[0].file.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0666);
                            dup2(fd2, 1);
                        }                        
                    }
                } else {
                    //n > 1
                    int _pos = 0;
                    if (redirs[_pos].pos != i) _pos++;
                    if (redirs[_pos].type == 0)
                    {
                        fd1 = open(redirs[_pos].file.c_str(), O_RDONLY);
                        dup2(fd1, 0);
                        if (i < n-1) dup2(pip[1], 1);
                    } else {
                        fd2 = open(redirs[_pos].file.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0666);
                        dup2(fd_in, 0);
                        dup2(fd2, 1);
                    }
                }
            ...
            //перобразование к c_str()
            vector<char *> c_args_ch;
            c_str_array(c_args_ch, args_ch[i]);


            if(execvp(c_args_ch[0], &c_args_ch[0]) == -1) perror("microsha");
